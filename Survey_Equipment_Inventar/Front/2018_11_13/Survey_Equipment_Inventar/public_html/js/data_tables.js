...
/**
 * Function to create table for searching products and filter it by column(s)
 * Using https://cdn.datatables.net/1.10.19/js/jquery.dataTables.min.js
 */
function table_search_products() {
    console.log('Initialize table for searching products.');
    var id = '#table_search_products';
...
function table_modal_single_product(id) {
    console.log('Initialize table ' + id + ' for modal of single product.');
    var table = $(id).DataTable({
        responsive: {
            details: {
                display: $.fn.dataTable.Responsive.display.childRowImmediate,
...
function table_select_location(product) {
    console.log('Initialize table for selecting the location for a new product.');
    // DataTable
    ...
    $('#add_table_location_body').on('click', 'tr', function () {
        if ($(this).hasClass('selected')) {
            ...
            if ($('#add_btn_toggle_borrowed_yes').hasClass('active')) {
                ...
                $('#add_new_product_confirm_location').text("");
...
function add_all_possible_locations(product) {
    ...
    $('#add_new_product_confirm_possible_locations').text(possible_locations_seperated_str);
...
function delete_all_possible_locations(product) {
    ...
    $('#add_new_product_confirm_possible_locations').text("");
    $('#add_btn_toggle_possible_locations_all').toggleClass('active');
    $('#add_btn_toggle_possible_locations_all').toggleClass('btn-toggle-off');
    $('#add_btn_toggle_possible_locations_all').toggleClass('btn-toggle-on');
    $('#add_btn_toggle_possible_locations_none').toggleClass('active');
...
function table_select_possible_locations(product) {
    console.log('Initialize table for selecting possible locations.');
    var clicked_rows = [];
    // DataTable
...
function add_loan_to_new_product(product) {
    if (product.loan_since === undefined || product.loan_since === "" ||
        ...
        $('#add_table_location_body').children().removeClass('selected');
        console.log('Product after adding loan.');
        console.log(product);
        update_table_confirm('#add_new_product_confirm_location', new_location);
        $('#add_btn_toggle_borrowed_yes').toggleClass('active');
        $('#add_btn_toggle_borrowed_yes').toggleClass('btn-toggle-on');
        $('#add_btn_toggle_borrowed_yes').toggleClass('btn-toggle-off');
        $('#add_btn_toggle_borrowed_no').toggleClass('active');
...
function remove_loan_to_new_product(product) {
    ...
    $('#add_new_product_confirm_location').text("");
    $('#add_table_location_body').children().removeClass('selected');
    console.log('Product after removing loan.');
    console.log(product);
    $('#add_btn_toggle_borrowed_yes').toggleClass('active');
    $('#add_btn_toggle_borrowed_yes').toggleClass('btn-toggle-on');
    $('#add_btn_toggle_borrowed_yes').toggleClass('btn-toggle-off');
...
function table_select_related_equipment(product) {
    console.log('Initialize table for selecting related products for a new product.');
    var clicked_rows = [];
    var clicked_rows_names = [];
    var clicked_rows_descriptions = [];

    // Setup - add a text input to each footer cell
    search_param = '#add_table_related_products'.concat(' tfoot th');
    // table_any_type tfoot th
    $(search_param).each(function () {
        //var title = $(this).text();
        $(this).html('<div class="input-group input-group-sm">'
    ...
    // DataTable
    var table = $('#add_table_related_products').DataTable({
        scrollY: '14em',
        paging: false,
        colReorder: true,
        info: false,
    ...
    $('#add_table_related_products_body').on('click', 'tr', function () {
        if ($(this.cells.item(4).children[0]).is(":focus") === true) {
            ...
            if (clicked_rows.length === 0) {
                ...
                delete product.related_products_names;
                delete product.related_products_descriptions;
                $('#add_new_product_confirm_related_products').text("");
                console.log('New product after delete its related products');
                console.log(product);
            } else {
                console.log('Selected related products for product:');
                console.log(clicked_rows);
                console.log(clicked_rows_names);
                console.log(clicked_rows_descriptions);
                product.related_products = clicked_rows;
                product.related_products_names = clicked_rows_names;
                product.related_products_descriptions = clicked_rows_descriptions;
                console.log('New product after update its related products');
                console.log(new_product);
                var related_products_str = product.related_products.toString();
                var related_products_str_split = related_products_str.split(",");
                var related_products_name_str = product.related_products_names.toString();
                var related_products_name_str_split = related_products_name_str.split(",");
                var related_products_seperated_str = related_products_str_split[0];
                related_products_seperated_str = related_products_seperated_str.concat(" (" + related_products_name_str_split[0] + ")");
                for (i = 1; i < related_products_str_split.length; i++) {
                    related_products_seperated_str = related_products_seperated_str.concat(', '
                            + related_products_str_split[i] + ' (' + related_products_name_str_split[i] + ')');
                }
                console.log('Update confirm table with related products');
                $('#add_new_product_confirm_related_products').text(related_products_seperated_str);
            }
        }
    });
}