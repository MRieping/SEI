    ...
    $(table_body_id).on( 'click', 'tr', function () {
        if ($(this).hasClass('selected')) {
            ...
            if(product.possible_locations === undefined) {
                product.possible_locations = clicked_row;
            } else {
                product.possible_locations.push(clicked_row);
            }
 ...
 * @param {array} clicked_rows_descriptions
 * @param {string} cell_id_confirm_table
 * @param {Object} product
 */
function table_select_related_equipment(table_id, table_body_id, clicked_rows, clicked_rows_names, 
    clicked_rows_descriptions, cell_id_confirm_table, product) {
    // Setup - add a text input to each footer cell
    ...
    $(table_body_id).on( 'click', 'tr', function () {
        console.log(this.cells.item(4).children[0].value);
        $(this).toggleClass('selected');
        var clicked_row = this.cells.item(0).innerHTML;
