...
function get_single_product(item_number, type) {
    var url_current = url;
    url_current = url_current.concat(type);
    url_current = url_current.concat('/');
    url_current = url_current.concat(item_number);
    ...
    $.ajax({
        ...
        success : function(result, status, xhr){
            if(result.length !== 0) {
                ...
                console.log(result);
                fill_tables_for_single_product(result, type);
            } else {
 ...
 * @param {String} item_number
 */
function delete_product(type, item_number) {
    var url_current = url;
    url_current = url_current.concat(type);
    url_current = url_current.concat('/');
    url_current = url_current.concat(item_number);
    $.ajax({
        ...
        contentType : "application/json; charset = utf-8",
        success : function(result, status, xhr){
            console.log("Status: " + status + " for deleting single " + type);
            console.log("with item number " + item_number);
        },
        error : function (xhr, status, error){
            console.log("Error");
            console.log("Status: " + status + "Error: " + error + " for deleting " + type);
            console.log("with item number " + item_number);
        }
    });
}