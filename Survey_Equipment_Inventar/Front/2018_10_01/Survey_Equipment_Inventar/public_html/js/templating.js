...
/**
 * 
 * @param {id} add_sensor_alert_error
 * @param {object} product
 */
function templating_add_sending(add_sensor_alert_error, product) {
    ...
    if(product.selected_location === undefined) {
        $(add_sensor_alert_error).text('Please select a location for the new product!')
        $(add_sensor_alert_error).fadeIn();
        setTimeout(function () {$(add_sensor_alert_error).fadeOut();}, 4273); 
    } else if(product.possible_locations === undefined) {
        $(add_sensor_alert_error).text('Please select at least one location where the product is allowed to store or to install!')
        $(add_sensor_alert_error).fadeIn();
        setTimeout(function () {$(add_sensor_alert_error).fadeOut();}, 4273); 
        ...
        $(add_sensor_alert_error).text('Please fill in all required fields for attributes!')
        $(add_sensor_alert_error).fadeIn();
        setTimeout(function () {$(add_sensor_alert_error).fadeOut();}, 4273); 
    } else if(product.images.length === 0) {
        $(add_sensor_alert_error).text('Please select at least one picture!')
        $(add_sensor_alert_error).fadeIn();
        setTimeout(function () {$(add_sensor_alert_error).fadeOut();}, 4273); 
        ...
        console.log('Try to send new sensor.');
    }
}
